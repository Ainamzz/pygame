        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r: #если 'r' цвет - красный
                    ...
                    draw_mode = 'right_triangle' #тікбұрышты үшбұрыш
                elif event.key == pygame.K_q:
                    draw_mode = 'equilateral_triangle' #теңқабырғалы үшбұрыш
                elif event.key == pygame.K_h:
                    draw_mode = 'rhombus' #ромб
...
def drawShape(screen, index, start, end, width, color_mode, draw_mode):
    ...
    if draw_mode == 'line':
        ...
        p1 = start
        p2 = (start[0] + width, start[1])
        p3 = (start[0], start[1] + width)
        pygame.draw.polygon(screen, color, [p1, p2, p3])
    elif draw_mode == 'equilateral_triangle':
        height = int(math.sqrt(3) * width / 2)
        p1 = (start[0], start[1] - height)
        p2 = (start[0] - width, start[1] + height // 2)
        p3 = (start[0] + width, start[1] + height // 2)
        pygame.draw.polygon(screen, color, [p1, p2, p3])
    elif draw_mode == 'rhombus':
        p1 = (start[0], start[1] - width)
        p2 = (start[0] - width, start[1])
        p3 = (start[0], start[1] + width)
        p4 = (start[0] + width, start[1])
